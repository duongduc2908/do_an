...
export default {
  ...
  async mounted () {
    try {
      await axios.get('http://localhost:4321/api/stream/connection_api/connect?rtsp_link=/media/server/A6F2-8B3D/CodeChuan/Core_Python_Flask/video1.mp4&username&password&selectedProtocol=1')
    } catch (error) {
      console.error(error)
    }
    ...
    setTimeout(async () => {
      await axios.get('http://localhost:4321/api/stream/connection_api/disconnect')
