...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async activateCamera (context, payload) {
      ...
      try {
        await axios.get(`http://localhost:4321/api/stream/connection_api/connect?place_id=${payload.placeId}&camera_id=${payload.cameraId}&rtsp_link=${payload.rtspLink}&username&password&selectedProtocol=1`)
      } catch (error) {
        console.error(error)
      }
    },
    ...
    async deactivateCamera (context, payload) {
      ...
      try {
        await axios.get(`http://localhost:4321/api/stream/connection_api/disconnect?place_id=${payload.placeId}&camera_id=${payload.cameraId}`)
      } catch (error) {
        console.error(error)
      }
    }
