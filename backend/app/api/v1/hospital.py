...
from app.enums import CREATE, DELETE, UPDATE, HOSPITAL, PATH_IMAGE_CAMERA
...
api = Blueprint('hospital', __name__)
...
def post():
    ...
    try:
        ...
        key = 'cameras'
        hospital = client.db.hospital.find_one({"_id": hospital_id})
        if hospital is not None:
            if key in hospital:

                hospital["cameras"].append(data["cameras"])
                update_cameras = {
                    '$set': {
                        "cameras": hospital["cameras"]
                    }}
                client.db.hospital.update_one({'_id': hospital_id}, update_cameras)
            else:
                cameras = []
    ...
    return send_result(message="Tạo thành công ", data=hospital)
...
def put():
    ...
    try:
        hospital = client.db.hospital.find_one({"_id": hospital_id})
        if hospital is not None:
            for camera in hospital["cameras"]:
    ...
    return send_result(message="Tạo user thành công ", data=client.db.hospital.find_one({'_id':hospital_id}))
...
def delete():
    ...
    try:
        ...
        if hospital is not None:
            ...
            for camera in hospital["cameras"]:
                if camera["camera_id"] == camera_id:
                    hospital["cameras"].remove(camera)
                    # Xoa file
                    try:
                        list_file = listdir(PATH_IMAGE_CAMERA)
                        for i in list_file:
                    ...
                    temp = 0
                    notif = notification(
                        content=claims["full_name"] + " đã xóa camera " + camera["name"] + hospital["name"],
...
def get_all_page_search():
    ...
    users = client.db.hospital.find().skip(skips).limit(int(page_size)).sort('_id', -1)
    '''end list'''
    list_user = list(users)
    totals = client.db.hospital.find().count()
    data = {
        'totals': totals,
        'results': list_user
    }
